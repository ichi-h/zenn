...
また、今回は 300ms → 80ns の速度改善ができましたが、この数値改善がビジネス的に必要なのかどうかは別の問題です。  
そもそも 300ms のスピードでユーザーが満足しているのであれば、このチューニングを行う必要はないともいえます。  
更に、このパフォーマンスチューニングを行った前後で、正確さに影響が出ないことを保証する必要があります。  
