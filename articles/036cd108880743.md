...
このように、tsuzuriでは具体的なUIを直接制御しない思想になっているため、トレードオフとして、**ステータスを通じて外部に何らかの処理を要求することがあります**。  
...
これをnext関数に対応させると、以下のようになります。

```tsx
const next = () => {
  if (model.status.value === "Delaying") return;

  // 参照するシナリオをcurrentScenarioで切り替える
  const message = scenario[model.currentScenario][model.index];
  if (!message) {
    console.log("シナリオ終了");
    return;
  }
...
ちなみに、`onClick` 等で発火していた `switchScenario({ scenario: 'XXX' })` というのは、シナリオ変更のメッセージを作成する関数です。
...
例えばゲームエンジン利用者が音声周りで扱いたいことは、**「BGMを再生したい」「ボイスを流したい」といった宣言的な命令**であるはずです。  
