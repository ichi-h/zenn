...
サーバーの電源を必要なときだけ稼働させておきたいという気持ちもあったので、外出の際は Raspberry Pi を常時稼働させておき、そこから各サーバーの電源を Wake-on-Lan で叩き起こせるようにしていました。
...
ところで、Nix は Docker のような仮想技術は用いておらず、パッケージを動作させるために必要なすべての依存関係をローカルにインストールし、それらのリソースを用いてパッケージを動作させます。  
これくらいシンプルな例であれば良いかもしれませんが、もしさらに複雑なパッケージ管理を必要とするプロジェクトの場合、仮想化なしでマシン間の動作差がない開発環境を用意できるのかという、至極最もな疑問が出てくると思います。

ですが、Nix はそうした問題は原理上発生しません。
...
また Worker Node や Control Plane で共通の設定というものも存在します。  
そうしたものも .nix ファイルとして切り出し、それぞれのサーバーで import してしまえば、同じ設定をすべてに適用することが可能です。  
私の環境では /base というディレクトリに全サーバー共通の設定をまとめ、それぞれで import するようにしています。
...
{
  imports = [
    ./core.nix # タイムゾーンやi18n等の共通設定
    ./docker.nix
    ./networking.nix # 主にfirewallの管理
    ./programs.nix # git, nvim, zshといったアプリケーションの有効化
    ./ssh.nix
    ./tailscale.nix
    ./users.nix
  ];
...
まとめると、**NixOS の再現性と宣言的という特性を活用することで、複数あるサーバーの個別ないし共通の設定を宣言的に管理しつつ、再現性のある環境構築を可能にしたかった**、ということです。

## サーバースペック

今回クラスターに参加させるノードは以下の 3 台です。  
（家のサーバーが多くなり、それぞれの区別が必要になったため、ホスト名に各サーバーの呼び名をつけています）

| Machine        | Hostname       | Role          | CPU            | RAM  | Storage                                |
| -------------- | -------------- | ------------- | -------------- | ---- | -------------------------------------- |
| Raspberry Pi 4 | yomogi（蓬）   | Control Plane | ARM Cortex-A72 | 4GB  | 64GB（SD カード）                      |
| Mini PC 1      | tokiwa（常磐） | Worker        | Intel N97      | 16GB | 512GB（M.2 SSD）                       |
| Mini PC 2      | yomogi（蓬）   | Worker        | Intel N97      | 16GB | 512GB（M.2 SSD）<br>1 TB（外付け SSD） |
...
NixOS と Kubernetes で異なった役割を持つため、それぞれを俯瞰する大まかなアーキテクチャ図を用いて簡単にお話いたします。
...
しかし NixOS でクラスターを立ち上げた場合、これらの要素は**すべて systemd のサービス**として立ち上がります。  
...
また上記のリストの通り、公式のパッケージを利用する場合 CNI は **Flannel 一択** となります。  
ホストで Flannel が立ち上がっていたとしても、仕組みは同様になりますので、詳細は割愛させていただきます。  
詳しくは以下の記事が参考になると思います。

https://zenn.dev/taisho6339/books/fc6facfb640d242dc7ec/viewer/0d112c

また `services.kubernetes.roles` の設定を行った場合、**easyCerts**というサービスが自動的に ON になります。
...
ただ、この IP アドレスを覚えるのは面倒ですし、またこの IP アドレスは（初期設定をいじっていなければ）定期的に変わるものなので、更新ごとに覚え直す手間が発生します。
...
一方、Tailnet に参加している端末がこのドメインにアクセスしようとすると、**Split DNS によって蓬サーバー（Raspberry Pi）から提供される Local DNS を名前解決に使用します。**  
...
おそらく NixOS + Kubernetes で環境構築した際に最もハマる沼は **Firewall** です。  
ここの設定をミスると、「なぜか Control Plane のノード内の Pod から kube-apiserver へのリクエストだけが通らない」などの謎現象が起こります。

色々と試行錯誤した結論としては、`10.0.0.0/8` と `192.168.10.0/24` からのリクエストを信頼する設定を追加してようやく動くようになりました（一応 Private IP Address といえど、かなり広い範囲を開けているので、より Better な設定はあると思います）。
...
```nix
{
  ...
  # containerdが同時に開けるファイルディスクリプタの上限を最大まで上げる
