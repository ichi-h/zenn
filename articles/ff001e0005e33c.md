...
すべてを読み通すのは大変だと思いますので、**気になる見出しからご覧いただくことを推奨します。**
...
ただし上記の通りにインストールを進めてしまうと、NixOS 23.11 が入ってしまいます。  
そのため、以下のビルドのリストから最新のイメージファイルを探してインストールすることを推奨します。
...
これはノード間で通信する際に使用する証明書を自動で発行・更新してくれる非常に便利な機能です。  
なのですが、公式の見解としては **「for a production-grade cluster you shouldn't use `easyCerts`（本番環境のクラスタでは easyCerts を使用すべきではありません）」** とのことなので注意が必要です（今回の開発では easyCerts を利用しています）。
...
他には Magic DNA という機能を用いて、Tailnet DNS name と各マシンのホスト名を組み合わせた `hostname.xxxx-xxxxx.ts.net` というドメインを使って通信する方法もあります。  
...
試行錯誤した結論としては、私の環境では `10.1.1.0/24` と `192.168.10.0/24` からのリクエストを信頼することで動くようになりました。
...
アプローチに差異はあれど、NixOS も Arch Linux もかなり自由度の高いディストリビューションでありますが、一点留意すべき点として、今回私は NixOS のパッケージを半ばプリセットのように使用しているという背景があります。  
...
基本的に NixOS では `inputs.nixpkgs.lib.nixosSystem` の引数として `system` を指定すれば、その CPU アーキテクチャに合わせて OS をビルドしてくれるのですが、一部 x86_64 にのみ対応する設定が存在しているため、その部分を arm64 向けに修正する必要があります。

今回は Control Plane で DNS を立ち上げる際に、参照するイメージの URL が x86_64 のものになっていたため、そのリンクの修正と、etcd に渡す環境変数の追加を行いました。
...
しかし一方で、これより先の領域は NixOS Wiki にも解説がなく、**ほとんど情報がない中手探りで環境構築を進めていくことが予想されるため、ただでさえ重い実装コストがさらに重くなる**というトレードオフが考えられます。
