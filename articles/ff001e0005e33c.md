...
Kubernetes のアーキテクチャは以下のとおりです。  
この章ではクラスター構築に用いたツールや、基盤を担うアプリケーションについて個別にお話できればと思います。
...
Kubernetes Dashboard のようなコンテナのデプロイが扶養で、CLI 上でクラスターの操作はもちろん、各 Pod のデプロイ状況やコンテナのログ監視までできるのは非常に便利です。  
...
また Rook Ceph には **Operator** と **Cluster** という 2 つの役割が存在します。  
といっても話は簡単でして、**Operator が Ceph Cluster の管理**を行い、**Cluster が実際のストレージ供給**を行っています。極論を言ってしまえば、**Operator が Rook、Cluster が Ceph** ということです。
