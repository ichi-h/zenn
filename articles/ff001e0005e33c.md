...
本記事では **[Kubernetes - NixOS Wiki](https://nixos.wiki/wiki/Kubernetes) の環境をベースに、自宅にあるサーバーを用いて Kubernetes クラスターの構築を行うこと**をテーマとしています。

本記事の著者は、これまでインフラやネットワークといった領域にそこまで深く踏み込んだことがなく、そのあたりの技術力に少し不満があったため、勉強がてらに元々あった自宅サーバーの環境を刷新しつつ、**ついでに NixOS + Kubernetes というロマンの塊のような技術スタックで作る自宅サーバーの簡単なガイドを作ってやろう**という目的のもと執筆を行っています。  
...
一方で、NixOS と Kubernetes という学習コストの高い技術を同時に扱う都合上、全体で短編小説ほどの文量がある技術記事となってしまっております。  
もちろん書くからにはその分の価値提供をできるよう最大限努力をしておりますが、長くなると読み手の負担が大きくなってしまうのも事実です。  
そのため本記事では、これ以上文量を増やさないために、**技術のミクロな使い方や導入方法を省略している箇所がございます**ので、この点はご容赦いただけますと幸いです。

代わりと言ってはなんですが、私のサーバー環境のすべては dotfiles で管理されています。  
詳細な解説ができなかった箇所については、本文中に `dotfiles dir` という形で設定したファイルへのリンクがありますので、そちらを参考にしていただければと思います。
...
- 参考
  - https://github.com/NixOS/nixpkgs/tree/7d5349a953107354a2357db80416b97dcf87073b/nixos/modules/services/cluster/kubernetes
  - https://github.com/NixOS/nixpkgs/blob/7d5349a953107354a2357db80416b97dcf87073b/nixos/modules/services/security/certmgr.nix
  - https://github.com/NixOS/nixpkgs/blob/3e2e68fb5b8ca7ed18f83385c2c9b82c8924ef61/nixos/modules/services/networking/flannel.nix
...
IP アドレスに比べれば覚えやすく、仮に IP アドレスが変わったとしても変化しない。そして**このドメインに対して TLS 証明書を発行できる**という状況によっては強力な機能を使うこともできます。
...
- **参考**
  - https://qiita.com/matorixmatrix/items/947e4e0039fd6aee9d65

### Proxy Server

上記のドメインの解決先となる花明かりサーバー（Mini PC 2）では、**Nginx** を用いて Node Port で公開された Pod へリバースプロキシを行っています。  
下記では 80 番と 443 番へのトラフィックを 31080 番と 31443 番へポートフォワーディングしておりますが、転送先はクラスター内で稼働する **Traefik** となっています（詳細は Kubernetes の章にてお話いたします）。
...
- **参考**
  - https://speakerdeck.com/shinbunbun_/nixosdemogozi-zhai-k8sgasitai
...
クラスター内でボリュームの永続化が必要になった際に、PVC にて要求された PV を供給するために **Rook Ceph** を用いました。
...
- **参考**
  - https://sqlengineer.hatenablog.com/entry/2024/07/20/105110
...
全く同じハードを使っているというのもありますが、理論をここまで正確に体現できるものなのかというのは、ある種の感動があります。
...
このように考えると、**再現性の高いパッケージを再利用して恩恵を得ようとするライトユーザーと、システムのすべてを宣言的に構築しようとするヘビーユーザーとの間で、NixOS の捉え方が変わる可能性がある**のかもしれません。

#### 悪かったところ（？） - arm64 について

前章で少しお話しましたが、今回 Raspberry Pi の CPU アーキテクチャが arm64 ということで、特別な設定をいくつか Nix 式上で行う必要がありました。  
...
こういった特殊なケースの考慮が発生するのは面倒ではあるのですが、この辺りはハードウェアの違いによる問題なので、この領域まで扱うのは NixOS といえどさすがに厳しいと思います（ハードウェアの違いは Nix 言語上では副作用となるため）。  
またどのディストリビューションであっても同様の問題は発生すると思うので、NixOS 固有の問題ではないかなと捉えています。
...
普通ならノード間の通信時にエラーが起きて稼働が止まりそうなものですが、Kubernetes は**放っておいても望ましい状態になるよういい感じに処理を続けてくれる**ので、そこの手間を考えなくて良いというのは非常に楽だなと思いました。
...
まず最大のネックは、**アーキテクチャのトリッキーさと、インフラの自由度の低さ**が挙げられると思います。
...
なので `services.kubernetes.roles` を使わず自前で環境を構築できれば、より自由で堅牢なクラスターを作れるんじゃないかなと思います。

しかし一方で、これは kubeadm ほどメジャーな技術選定ではなく、**ほとんど情報がない中手探りで環境構築を進めていくことが予想されるため、その分の実装コストが増えていく**というトレードオフが考えられます。

まとめると、`services.kubernetes.roles` を使うにしても使わないにしても、それぞれに割と無視できないレベルの辛みがあり、また Kubernetes に重点を置くならば、そもそも NixOS を使わずともよりベターな解決方法が考えられるため、基本的に **NixOS + Kubernetes による環境構築はおすすめすることができません。**
...
クラスターを立ち上げから利用まで乗せることができたものの、また安定した状態にはなっておらず、適当にデプロイをするとどこかがコケるというケースがそれなりにあります。  
