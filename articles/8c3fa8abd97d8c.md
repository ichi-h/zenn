...
詳細に書こうとするとこの場では収まりきらなくなってしまうので、要点だけばっと述べさせてください。
...
そこで近年注目されていたのが**関数型プログラミング**というパラダイムであり、**副作用を持たず、参照透過性が保証された関数によって状態を評価することにより、コードの予測性やTestabilityを高めよう**としたと記憶しています。
...
さらに、@ichi-h/elmishがアプリケーションロジックに干渉するのは、Update関数やMessageの**型のみ**であり、それ以外でライブラリがロジックに干渉することはありません。  
...
もちろんこれは自画自賛したいのではなく、もともとは**Elm Architectureという偉大な設計を生み出したコミュニティの功績**です。  
私のライブラリも触ってほしいですが、それよりもElmを触っていただけた方が、私としては非常にうれしく思います。
...
const { useElement, send } = elmish<Model, Message, Html>(renderer);
